...
**Version:** 0.1 (Draft)  
**Date:** 2026-03-12  
**Amendments:** DM Channel Type and Compliance Logging patch (§9.6, §17.1, §17.2, §17.5, §17.7, §17.8)
...
**The recovery key is the root of trust for identity recovery.** Its private key MUST NOT leave a secure offline boundary — paper backup, air-gapped device, or hardware security key. It MUST NOT be imported into any networked device or software keystore under any circumstances during normal operation. "Not networked" is not a sufficient condition for safe storage; the key MUST NOT be extractable from whatever medium holds it.
...
- The MLS group state exported via the MLS key export interface (RFC 9420 §8) — this is a derived exportable blob, not raw key material
- Public keys and DID document
- Timestamp and DID

A backup from a non-extractable-key environment restores MLS continuity (epoch state, group membership, cached messages) but cannot restore the private keys themselves. Restoring identity on a new device requires either (a) the social recovery mechanism (§2.4.2), which reconstitutes the signing key, or (b) generating a new device keypair and having an existing device add it as a new MLS leaf — the standard multi-device enrollment flow.

Clients SHOULD clearly communicate to the user which type of backup their platform produces. A backup that omits private keys is not a full identity backup; users must understand they need a separate recovery mechanism for the keys.

The backup file is self-describing: it includes the Argon2id parameters, the DID, and a `backupType` field (`"full"` or `"stateOnly"`), so a client can prompt for the passphrase and restore appropriately without requiring additional configuration or external lookup.
...
The provisioning service MUST record the creation timestamp and provisioning operator identity in an audit log. This establishes the chain of custody for the identity from its inception.
...
**Step 2 — IdP authentication.** The client initiates an SAML 2.0 or OIDC authentication flow against the organization's IdP. The user authenticates (password + MFA) and the client receives an IdP token (SAML assertion or OIDC `id_token`) proving the user's identity to the organization.
...
`existingDeviceSig` is a signature over the canonical serialization of the other fields, made by an existing registered device key. For first-device enrollment it is absent. For subsequent devices it MUST be present — the IdP token alone authorizes adding a device only on first enrollment; subsequent enrollment requires proof of an existing authorized device (to prevent a compromised IdP token from unilaterally adding attacker-controlled keys to an already-provisioned account).

`secureEnclaveAttested` is `true` if the platform supports hardware attestation of key generation (Apple Secure Enclave via DeviceCheck, Android StrongBox via Android Key Attestation). When `true`, the provisioning service SHOULD verify the attestation before accepting the key. Attestation verification is platform-specific and optional for implementations that do not require hardware key provenance guarantees.

**Step 4 — Provisioning service validation.** The service:
1. Verifies the IdP token signature against the IdP's public key (fetched from IdP metadata endpoint)
2. Verifies the token is issued for the user matching the `did` in the request (`sub` claim in OIDC; `NameID` in SAML)
...
`deviceKeyFingerprint` is the first 16 bytes of the SHA-256 hash of the Ed25519 public key, hex-encoded. This produces stable, unique fragment identifiers without embedding the full key in the fragment.

**Step 6 — Gossip publication.** The provisioning service gossips the updated DID document hash on `v1/agora/discovery` so relays and peers learn of the new key without waiting for cache expiry.
...
`requestingSig` MUST be a signature by a currently registered device key that is not the one being revoked.

**Organization-initiated revocation.** An IT administrator authenticates to the provisioning service admin interface (separately authorized; requires an admin role in the IdP) and submits a revocation for any device key or for the entire DID (full account suspension):
...
`revokeAll: true` removes all device keys from the DID document, effectively suspending the account. The DID document remains resolvable (the org does not delete it) but has no `authentication` keys, so no operations can be authenticated against it.
...
Relays that receive a valid `DeviceRevocation` message MUST immediately invalidate any cached session tokens for the revoked key ID (§8.6). The relay MUST NOT accept new connections authenticated with the revoked key after receiving this message, regardless of DID document cache TTL.

**MLS remediation.** Revocation of a device key does not automatically remove that device's MLS leaf nodes from all groups — that requires MLS `Remove` commits from current group members. The revoked device's client (if still running) SHOULD issue `Remove` self-proposals on all its group memberships. If the device is genuinely lost, other group members MUST issue `Remove` commits when they detect the key is revoked (by observing the `DeviceRevocation` gossip message or a DID document refresh showing the key absent). Guild administrators SHOULD run periodic reconciliation to ensure revoked device keys have no active MLS leaves in their channels.

#### 2.5.6 Offboarding

When an employee leaves, the organization performs a full account suspension:

1. IdP account is disabled (blocks new IdP token issuance — no new device enrollments possible)
...
5. The organization retains the DID document (does not delete it) for the regulatory retention period — the DID is a permanent part of compliance records
6. After the retention period, the document MAY be blanked (empty `verificationMethod`) but MUST NOT be removed — the URL must remain resolvable and return a valid document to avoid dangling references in archived compliance records

The DID must remain resolvable indefinitely because compliance archives (§17, RTE bridge) contain `X-Agora-Sender-DID` references to it. An unresolvable DID in a compliance record breaks the chain of custody for that record.

#### 2.5.7 Provisioning Service DID Document

The provisioning service itself is a DID-bearing principal. Its DID document is published at a well-known location and its signing key is used to countersign DID document updates:
...
Every DID document update the provisioning service publishes MUST include a `provisionerSig` field: an Ed25519 signature over the canonical CBOR serialization of the updated DID document, signed by the provisioning service's key. Clients and relays MUST verify this signature when processing DID document updates received via gossip, using the provisioning service DID resolved from the user's DID document `controller` chain.
...
- **Recovery key (§2.4.1):** Valid, but the recovery key can only rotate device keys — it cannot override an organization-initiated `revokeAll`. An org-suspended account remains suspended even if the user holds a recovery key. Recovery keys are appropriate for individual-initiated recovery (lost device, not offboarding).

- **Social recovery (§2.4.2):** Valid for user-initiated recovery. Guardians can be other employees or external trusted parties. The provisioning service accepts a `RecoveryAssertion` signed by the social recovery reconstructed key as equivalent to an IdP token for first-device enrollment, provided the `recoveryCommitment` in the DID document verifies.

- **Encrypted backup (§2.4.3):** Valid. The backup restores MLS state and device keys. The restored device still needs to be enrolled via §2.5.4 to update the DID document with the restored public key (or the backup includes a currently-registered key whose registration already appears in the DID document, in which case no re-enrollment is needed).

- **Org-initiated recovery (helpdesk):** When a user loses all devices and has no recovery key or guardians, the organization can initiate recovery on their behalf: an admin issues a provisioning request (with `existingDeviceSig` absent, substituted by admin authorization) to add a new device key to the user's DID document. This is the enterprise equivalent of "call IT to reset your password." It requires the same admin authorization as §2.5.5 revocation. This path MUST be logged in the audit trail — it is a privileged operation.
...
- Small guilds where channel membership changes are infrequent and users tolerate occasional Welcome delays.

Best-effort mode is **not appropriate** for large public guilds or any deployment where a user may be offline for days or weeks and needs to be addable to new channels on their return. For those cases, a service agreement with a `keyPackageSLA` clause is required.
...
**`minimumCount`** — the relay commits to maintaining at least this many live, unconsumed KeyPackages per DID it serves under this agreement. This is not a hard real-time guarantee (the relay cannot generate KeyPackages itself) but an obligation to alert the client promptly when supply drops and to attempt IPFS-based replenishment (see below) before returning 404 on a fetch.
...
- `"push"` — the relay sends a push proxy wake-up (§8.7) with `urgency: "high"` addressed to the DID's registered `pushHandle`.
- `"gossip"` — the relay publishes a `KeyPackageLowAlert` message to `v1/agora/discovery`. The client picks this up on next connection.
...
`KeyPackageLowAlert` is gossiped only to `v1/agora/discovery`, not to any channel topic — it contains no channel or guild information. The target DID's clients subscribe to discovery and see the alert on next connection.
...
**`replenishFromIPFS`** — if `true`, the relay will attempt to fetch fresh KeyPackages from the owning DID's declared IPFS supply endpoint (§6.1.1, IPFS Supply below) before returning 404 on a fetch or before triggering an alert. This is the relay self-healing from IPFS without requiring the user to be online.

Violation of a `keyPackageSLA` clause (e.g., the relay returns 404 without attempting IPFS replenishment when `replenishFromIPFS: true`) constitutes a breach of the service agreement. Enforcement is by relay selection — guild owners and users choose relays that honor their declared terms.
...
The relay MUST verify the imported KeyPackages (same validation rules as §6.1.1 KeyPackage Validation) before adding them to the store. Corrupted or expired packages from IPFS are silently dropped; only valid packages are stored.
...
The IPFS batch is a **reserve tank** — the relay draws from it when the fast-path store runs low, without requiring the client to be online. When the client next connects, they check their count via `GET /count`, see it has been partially consumed, and generate a fresh IPFS batch to replace it.

**Supply depletion.** If both the relay store and the IPFS supply CID are exhausted (all packages consumed, no new CID published), the `KeyPackageRequest` gossip mechanism takes over as the last resort — same as today. IPFS supply does not eliminate this path; it makes it significantly less common.
...
The `forward=true` parameter instructs the Relay to attempt peer-relay fetching before returning 404. If `replenishFromIPFS` is also enabled (via service agreement), the relay attempts IPFS replenishment after peer-relay fetching fails, before finally returning 404. Relays that support forwarding declare `"keypackage-forwarding-v1"` in their capabilities.
...
`KeyPackageRequest` messages are ephemeral (not stored in IPLD history) and MUST NOT identify which channel or guild triggered the request — only that the target's KeyPackages are needed. With IPFS supply in place, a `KeyPackageRequest` reaching the relay may be satisfied immediately by the IPFS replenishment flow above, without waiting for the user to come online, if the IPFS supply CID still has unconsumed packages.
...
A Welcome constructed with an invalid or expired KeyPackage MUST be rejected. A Welcome constructed using a KeyPackage sourced directly from an IPFS supply CID (rather than consumed via the relay store API) MUST also be rejected — the consumption path matters, not just the package content.
...
`sig` is the pinner's Ed25519 signature over the canonical CBOR serialization of the pin entry fields (excluding `sig` itself), signed by the pinner's authentication key. Pins are attributable and tamper-evident — adding or removing a pin without a valid signature MUST be rejected.

`pinLimit` is the maximum number of simultaneous pins for the channel. Default is 50. Guild Admins may set a different limit via a signed `ChannelStateMutation`. Relays MUST reject `PinEvent` messages that would cause the pin count to exceed `pinLimit`.

#### Authorization

- **Any current MLS group member** MAY add a pin (`PinEvent`).
- **Guild/Channel Admins and the original pinner** MAY remove a pin (`UnpinEvent`). Other members MAY NOT remove pins they did not add.
...
Both are stored in the channel's IPLD history DAG — the audit trail records who pinned and unpinned what and when, non-repudiably. Clients update the `pins` array in local channel state on receipt of a valid `PinEvent` or `UnpinEvent` and propagate the updated channel state CID.
...
The relay MUST continue to serve pinned CIDs via IPFS until they are explicitly unpinned via a valid `UnpinEvent`. If a relay is running low on storage, it MUST evict unpinned content before considering eviction of pinned content, and MUST alert the guild operator before evicting anything from the pin list.

The retention exemption applies only to the pinned CID itself — not to the full IPLD history DAG in which that CID is embedded. Relays are not required to retain the entire history around a pinned message; they are required to retain the pinned CID and its directly referenced content (e.g. a media attachment CID referenced by a pinned message envelope).
...
Clients MUST support CommonMark rendering. Clients that cannot render CommonMark MUST display the `text` field verbatim — CommonMark source is human-readable as-is, so degraded display is acceptable.

**Permitted CommonMark features:** All constructs defined in the CommonMark spec are in-scope. No extensions are included in the baseline. In particular, the following are explicitly **not** in the CommonMark baseline and MUST NOT be rendered unless negotiated as an extension (§6.7.2):
...
Clients MUST strip or escape any raw HTML in a CommonMark `text` field rather than rendering it. CommonMark's spec permits raw HTML passthrough; Agora explicitly disables it at the application layer. A `text` field containing raw HTML is not a conformance violation at the protocol layer, but clients MUST treat it as literal text.
...
Reserved extension format strings (for future standardization):
...
Senders choosing a non-baseline format SHOULD verify recipient capability before sending. The capability advertisement mechanism is defined in §6.7.4.

#### 6.7.3 Compliance Logger Behavior

Compliance loggers (§17) store the raw `text` field as-is in `plaintextPayload`. The `format` field is preserved alongside it. Loggers are not required to render or interpret the markup — the stored `text` is the authoritative record. Search and eDiscovery tooling operating on the compliance archive MUST be able to handle CommonMark source as plaintext (which it is legible as) or strip markup before indexing.
...
This declaration is queryable by any sender before composing a message, including when the recipient is offline. It reflects the capabilities of the user's preferred client. Users who switch clients SHOULD update this entry to match the new client's capabilities. The entry is informational — senders SHOULD use it to choose a format the recipient can render, but messages sent in an unsupported format are not a protocol violation.

**ClientHello declaration (ephemeral)**

The `supportedTextFormats` field in `ClientHello` (§8.6) declares the current connected device's rendering capabilities for the duration of the session. This is more precise than the DID document entry (it reflects the specific device and client version currently connected) but is only available while the client is online.

On receiving a `ClientHello` with `supportedTextFormats`, the Relay SHOULD cache this information keyed by DID + deviceKey for the session duration and make it available to other authenticated clients via the capability query endpoint:
...
Relays MUST NOT cache `ClientHello` capability data beyond the session. On disconnect, the cached entry is discarded. Subsequent queries fall back to the DID document entry or 404.

**Sender behavior**

When composing a message with a non-baseline `format`, a conformant client SHOULD:

1. Query `/v1/agora/caps/{did}` for each recipient (in DM groups) or check the DID document (in guild channels, where per-member querying at compose time is impractical).
...
In guild channels with many members, per-member capability queries at compose time are not required. Clients MAY display a warning when a non-baseline format is selected (e.g., "Some members may not render GFM tables") without blocking the send.

**Multi-device considerations**

A user's devices may have different rendering capabilities (e.g., a feature-rich desktop client and a minimal mobile client). The DID document entry SHOULD reflect the union of all active devices' capabilities — declaring a format only if all devices the user actively uses can render it. The `ClientHello` entry reflects the specific device currently connected and MAY differ from the DID document entry.

### 6.8 Emoji and Reactions

#### 6.8.1 Emoji in Message Text

Unicode emoji are valid UTF-8 content in any `TextBody` `text` field. No special handling is required — they are codepoints like any other. Clients MUST render them using the platform's native emoji font or a bundled emoji font; the protocol does not specify a vendor or version.

**Shortcodes** (`:thumbsup:`, `:wolfssl_logo:`) are a client-side rendering convenience, not a wire format. Shortcodes MUST NOT appear on the wire in `TextBody.text` unless the sender intentionally wants the literal text `:thumbsup:` to appear. Clients that support shortcode input MUST expand shortcodes to their Unicode codepoint(s) (for standard emoji) or to a custom emoji reference (for guild emoji, see §6.8.4) before constructing the `TextBody`. The mapping from shortcode to codepoint is a client implementation detail; the protocol does not mandate a specific shortcode dictionary.
...
**One reaction per emoji per sender per target.** A sender MAY NOT add the same `emoji` value to the same `targetCID` more than once. A second `"action": "add"` for the same `(senderDID, targetCID, emoji)` tuple is a no-op; clients MUST deduplicate and MUST NOT display it as a separate reaction. A `"action": "remove"` with no prior `"add"` for the same tuple is also a no-op.
...
A sender may toggle a reaction by sending `"add"` followed by `"remove"`. There is no `"toggle"` action — clients MUST send explicit `"add"` or `"remove"`.

**Aggregation.** Clients SHOULD aggregate reactions by `emoji` value, displaying a count and (on hover or tap) the list of sender display names. The display order of distinct emoji types is client-defined. A reaction count of zero MUST NOT be displayed — once all senders have removed a reaction, it disappears from the UI.

**Reactions on edited messages.** A `targetCID` always refers to the original message envelope CID, not to an `EditEvent` CID. Editing a message does not reset or invalidate its reactions. Clients display reactions on the current (most recently edited) version of the message body.
...
**Reactions and compliance logging.** `ReactionEvent` messages are logged by the compliance logger identically to `ChatMessage` messages — they are MLS application messages and the logger holds group membership. Compliance with §17 applies to reactions.

#### 6.8.4 Custom Emoji

Guilds MAY define custom emoji — guild-specific images referenced by shortcode. Custom emoji are IPFS-hosted and declared in the guild state document.

**Guild state declaration:**
...
`shortcode` is the guild-local name for the emoji, used in the `:guild:<shortcode>:` reference format. It MUST match `[a-z0-9_]{1,32}` — lowercase alphanumeric and underscores, 1–32 characters. Shortcodes MUST be unique within a guild. `imageCID` is the IPFS CID of the image file. `mimeType` MUST be `image/png`, `image/gif`, or `image/webp`. `width` and `height` are in pixels; images SHOULD be square and SHOULD be 64×64 or 128×128 pixels. Animated GIF and WebP are permitted for animated emoji.
...
```json
"emoji": ":guild:wolfssl_logo:"
```

Clients resolve `:guild:<shortcode>:` by looking up `shortcode` in the current guild's `customEmoji` array, fetching the image from IPFS at `imageCID`, and rendering it inline. Clients that cannot resolve the shortcode (unknown guild, image unavailable) MUST display the literal reference string `:guild:wolfssl_logo:` as a fallback.

**Custom emoji in message text:**

Custom emoji MAY appear inline in `TextBody.text` using the same `:guild:<shortcode>:` reference syntax. Clients resolve and render them inline alongside the surrounding text. This is a client rendering responsibility — the wire format carries the literal reference string in the `text` field, and CommonMark processors that do not understand custom emoji references will display the literal text, which is acceptable degraded behavior.

**Custom emoji management:**

Adding or removing a custom emoji is a signed `GuildState` mutation by a Guild Admin or Owner. The `customEmoji` array in the guild state document is the authoritative list. Removing a custom emoji from the array does not delete the IPFS content (CIDs are immutable and may be pinned elsewhere), but clients MUST stop rendering it after the next guild state refresh. Existing messages containing references to a removed emoji display the fallback literal string.

Guilds SHOULD pin custom emoji image CIDs to prevent relay eviction. Custom emoji images are small and the pin cost is negligible.
...
`supportedTextFormats` is an optional array of `TextBody` format strings (§6.7) that this client can render. If absent, the client is assumed to support only `text/markdown;variant=commonmark` and `text/plain`. The Relay does not use this field — it is included in `ClientHello` so that the Relay can gossip it to other connected clients who are composing messages to this DID (see §6.7.4). Clients SHOULD include this field on every connection.
...
In this mode:
- Each participant derives a media encryption key via the MLS exporter interface: `MLS-Exporter("agora-media-key", participantDID, 32)` (RFC 9420 §8.5). The epoch secret never leaves the MLS stack; the exporter output is the only material exposed to the Insertable Streams layer.
...
This section fully specifies the `RecordingGrant` mechanism referenced in §10.6 — the key delivery sub-protocol, the recording archive format, participant disclosure requirements, and compliance recording configuration. It supersedes the placeholder in §10.6 and resolves the open question in Appendix B item 1.
...
`grantedTo` is the DID of the recording principal — either the guild's SFU DID (for SFU-mediated recording) or a dedicated compliance recorder DID (for compliance logging). `scope` is `"audio-only"`, `"video-only"`, or `"audio-video"`. `purpose` is `"operational"` (ad-hoc recording by an admin) or `"compliance"` (regulatory retention obligation). `participantDisclosure` is a human-readable string that MUST be displayed to all channel participants before recording begins. `expiresAt` sets the maximum grant duration; compliance grants SHOULD NOT have an `expiresAt` earlier than the guild's `retentionDays` from the grant timestamp.
...
`grantCID` is the IPFS CID of the `RecordingGrant` being revoked. Both `RecordingGrant` and `RecordingRevoke` are stored in the channel's IPLD history DAG — the full grant and revocation history is permanently auditable.

#### 10.12.2 Key Delivery Sub-Protocol

The recording key is derived from the MLS exporter interface and delivered to the recorder's DID via an encrypted `RecordingKeyDelivery` message. This is the sub-protocol that was previously underspecified.
...
`encryptedKey` is the 32-byte `recordingKey` encrypted to the recorder's X25519 `keyAgreement` key from its DID document, using HPKE (RFC 9180, DHKEM(X25519, HKDF-SHA256), HKDF-SHA256, AES-128-GCM). The HPKE `info` parameter is `"agora-recording-key-delivery-v1" || grantCID_bytes`. This ensures only the recorder's private key can unwrap the recording key.
...
The recorder collects `RecordingKeyDelivery` messages from each participant. It does not need all participants to deliver — it needs only one valid delivery per epoch to decrypt that epoch's media. However, clients MUST NOT withhold delivery; failing to deliver is a protocol violation for a participant who is in the call.

**Epoch gaps.** If the recorder misses a `RecordingKeyDelivery` for an epoch (e.g., it was offline), it cannot decrypt media from that epoch retroactively. The `RecordingComplianceAuditEntry` (§10.12.5) records any epoch gaps detected by the compliance logger, so the regulated organization has a complete record of any capture gaps.

#### 10.12.3 Participant Disclosure

Before recording begins, participating clients MUST display the `participantDisclosure` string from the `RecordingGrant` as a non-dismissable system message in the voice channel's text feed and as a visible indicator in the call UI for the duration of the recording. The indicator MUST persist for the full duration of the grant — it MUST NOT disappear after initial acknowledgment.
...
Participants MAY leave the call at any time. Their media prior to departure is retained in the recording archive for the grant's retention period.

For compliance grants (`"purpose": "compliance"`), the disclosure MUST include the `loggerLabel` and `retentionDays` from the guild's `ComplianceLoggingConfig`:
...
`mediaCID` is the IPFS CID of the recorded media file — the decrypted, re-encoded audio/video archive. `mediaFormat` is a MIME type string. `participants` lists every DID that was in the call during the epoch, with join and leave timestamps derived from `ParticipantState` gossip events. `prevRecordCID` chains records into the same append-only IPLD structure used for text compliance (§17.4), making the recording archive tamper-evident. `recorderSig` is the recorder's Ed25519 signature over the canonical CBOR serialization of all other fields.

Media SHOULD be stored as Opus audio in an Ogg or WebM container (audio-only recordings) or VP8/VP9/AV1 video with Opus audio in a WebM container (audio-video recordings). The recorder MUST decode the SRTP streams using the delivered recording keys and re-encode to the archive format before writing to IPFS. The raw SRTP packets MUST NOT be stored — storing encrypted media that cannot be decrypted without the recording keys defeats the purpose of the archive.

Per-epoch media files are stored separately (one `mediaCID` per epoch) and linked by the `VTCComplianceRecord` chain. This limits the blast radius of a failed epoch: a missed key delivery for one epoch produces a gap in that epoch's record, but all other epochs are unaffected.

#### 10.12.5 VTC Compliance Configuration

To enable automatic compliance recording for a guild's voice channels, the guild owner adds a `vtcCompliance` block to the guild state document alongside the `complianceLogging` block:
...
`recorderDID` is the DID of the compliance recording principal — may be the same DID as `complianceLogging.loggerDID` or a separate one. `autoGrant` when `true` causes conformant clients to automatically issue a `RecordingGrant` for `recorderDID` on every voice channel in the guild when a call begins, without requiring per-call admin action. This is the compliance equivalent of the text logger being automatically added to channels when `complianceLogging.enabled` is set.

When `autoGrant` is `true`, the `RecordingGrant` is issued by the first Admin or Owner client that observes the call starting (i.e., the first `ParticipantState` with `status: "joined"` on a voice channel with no active grant). The grant `expiresAt` is set to the call end — detected when all `ParticipantState` entries for the channel reach `status: "left"` or TTL expiry — plus a configurable grace period (default 5 minutes) to capture any reconnecting participants.

The `VTCComplianceConfig` is gossiped with the guild state. A recorder whose DID is declared in `vtcCompliance.recorderDID` MUST have a `KeyPackageEndpoint` service entry in its DID document (same requirement as the text compliance logger, §17.8.2) and MUST pre-upload an HPKE public key in its DID document's `keyAgreement` entry so participants can encrypt `RecordingKeyDelivery` messages to it.

#### 10.12.6 VTC Compliance Audit Entry

The guild's IPLD compliance audit chain receives a `VTCComplianceAuditEntry` for each compliance recording session:
...
`epochsGapped` is an array of epoch numbers for which no `RecordingKeyDelivery` was received, resulting in an unrecorded segment. An empty array means complete capture. Non-empty arrays are compliance gaps that must be reported to the compliance officer. `vtcRecordCIDs` links to the `VTCComplianceRecord` chain for this session.

#### 10.12.7 VTC Compliance in DM Voice Groups

A DM group (§9.6) may include a voice channel component. The obligation from §17.8 extends to voice: a regulated Guild member participating in a DM voice group MUST add the guild's `recorderDID` to the DM group's MLS group and issue a `RecordingGrant` for the recorder before any voice media is transmitted, following the same mechanics as §10.12.2. The `DMGroupDescriptor` `complianceLoggers` array records both text logger DIDs and VTC recorder DIDs.

Participant disclosure (§10.12.3) applies in DM voice groups identically to guild voice channels. Non-regulated DM participants receive the disclosure notice and MAY leave before transmitting media.
...
`keyPackageSLA` is an optional clause that elevates the relay's KeyPackage Store behavior from best-effort to a contractual obligation (§6.1.1). It is omitted entirely in service agreements for deployments that operate in best-effort mode. When present, all fields are required and the relay's conformance to them is a term of the agreement. See §6.1.1 for the full semantics of each field.
...
- **Authentication key signing** (Ed25519 / P-256) — MUST use hardware-backed key where available. On platforms that do not support Ed25519 in the secure boundary (notably Secure Enclave, which supports P-256 only), P-256 (`ES256`) MAY be used as the authentication key algorithm for that device. The DID document MUST reflect the actual key type in use; the protocol is algorithm-agnostic for the authentication key role.

- **MLS leaf operations** (KeyPackage generation, Commit signing, Update proposals) — MUST use keys held in the secure boundary. MLS client libraries that accept a PKCS#11 or platform key handle for leaf signing are preferred. `openmls` supports pluggable crypto backends; implementations SHOULD provide a backend that delegates to the platform keystore rather than holding raw key bytes.

- **MLS key schedule / epoch secrets** — these are MLS-internal derived values, not persistent keys. They exist transiently in MLS stack memory during epoch processing and SHOULD NOT be exposed to application code. All operations requiring epoch secret access (media key export, recording key export, channel token derivation) MUST use the MLS exporter interface (RFC 9420 §8.5) rather than accessing the epoch secret directly.

- **X25519 key agreement** (for encrypting social recovery shares, HPKE operations) — SHOULD use hardware-backed key agreement where the platform supports it. Secure Enclave supports P-256 ECDH; for X25519 specifically, a hardware security key (YubiKey 5 series) or a TPM with X25519 support is required. Where hardware X25519 is unavailable, software implementation in a sandboxed process (isolated from the main application process) is the fallback.
...
**What the spec does NOT require from application code:** application code MUST NOT have access to raw private key bytes for any of the above. API calls should be of the form "sign this payload with key handle K" or "agree on a shared secret using key handle K and this public key", with the output being a signature or shared secret — not a key. Implementations that store raw Ed25519 or X25519 private key bytes in a variable, struct field, or configuration file are non-conformant with this guidance.

**Relay operator key storage:** Relay signing keys (used for manifest signatures and HTTP message signatures per §3.5.3 and §3.5.4) SHOULD be held in an HSM or TPM-backed keystore on the relay host. A relay operator's signing key is a long-lived organizational key; its compromise enables manifest forgery and peer impersonation. Cloud HSM services (AWS CloudHSM, GCP Cloud HSM, Azure Dedicated HSM) are appropriate for hosted relay deployments. At minimum, keys SHOULD be stored in the OS keyring (Linux `kernel keyring` / `systemd-credentials`, macOS Keychain) rather than on-disk in plaintext.
...
In all of these systems, impersonation by the organization leaves no mandatory cryptographic trace in the messages themselves. Audit logs may record administrative actions, but those logs are also under organizational control.
...
- **Per-message signing:** Every Agora message is signed by the sender's device key. The signing key fingerprint is embedded in the message's MLS authenticated data — it is bound to the ciphertext and cannot be altered without invalidating the message's authentication tag. The fingerprint is also derivable from the sender's DID document at the time of sending, providing a chain from message → key fingerprint → DID document entry → device.
...
This property is relevant for FINRA Rule 17a-4, SEC Rule 17a-4, and MiFID II recordkeeping requirements, where the integrity and authenticity of retained communications are material. Agora's architecture provides a stronger authenticity guarantee for archived communications than any platform that relies on organizational CA or IdP infrastructure alone.
...
VTC compliance recording is governed by the `RecordingGrant` mechanism and the `VTCComplianceConfig` guild configuration, both fully specified in §10.12.

**Relationship to text compliance logging.** Text compliance logging (§17.1–§17.5) and VTC compliance recording (§10.12) are parallel mechanisms operating over the same MLS group. They are separately authorized and separately archived, but both feed into the same guild IPLD compliance audit chain. A guild with both `complianceLogging.enabled: true` and `vtcCompliance.enabled: true` captures the complete communication record — text, reactions, edits, deletions, and voice/video — in a unified, tamper-evident audit chain.

**Scope.** §17.1–§17.5 cover: text messages in guild channels, reactions, edit events, delete events, file attachments, and DM text messages for regulated members (§17.8). §10.12 covers: audio and video media streams in guild voice channels and DM voice groups for regulated members (§10.12.7).

**Same logger DID or separate.** The `complianceLogging.loggerDID` and `vtcCompliance.recorderDID` MAY be the same DID or different DIDs. Using the same DID is operationally simpler but requires the compliance principal to support both text archiving (receive-only MLS membership) and VTC recording (HPKE key receipt and media decryption). Using separate DIDs allows specialist infrastructure for each function. Either configuration is conformant.
...
1. **Cross-guild identity and reputation** — there is currently no mechanism for carrying moderation history or reputation across Guild boundaries. A user banned from one Guild can freely join another. Whether this is a feature (clean-slate by design, prevents monoculture moderation) or a gap (enables bad actors to evade consequences) depends on use case. A voluntary cross-guild reputation attestation format is worth considering as an optional extension.
...
3. **Slow transport extensions** — Agora's message envelopes are transport-agnostic CBOR blobs and can be carried over store-and-forward transports for air-gapped, firewalled, or intermittently-connected deployments. Two extensions are planned: an SMTP gateway (relay accepts inbound SMTP, bridges to gossipsub; header privacy rules apply) and a batch store-and-forward format compatible with UUCP, rsync, sneakernet, HF radio, and similar transports. Both extensions apply to application messages only — MLS handshake messages (Welcome, Commit, Proposal) require ordered timely delivery and MUST use the primary gossipsub path. Capability strings: `slow-transport-smtp-v1`, `slow-transport-batch-v1`.

---

## Appendix C: Version Policy
