...
All versioned protocol surfaces in this specification are at **v1** for the initial release. The version appears as a path prefix on HTTP endpoints (`/v1/agora/...`) and as a topic prefix on gossip topics (`v1/agora/...`), JSON-LD context URLs (`/ns/v1`), and document `schemaVersion` fields. A breaking change to any surface increments only that surface's version; other surfaces are unaffected. Clients and relays MUST reject messages or connections whose version they do not support and MUST NOT silently coerce unknown versions.
...
- **Gossip topics** — version is in the topic string; v1 and v2 topics coexist on the mesh; a relay that supports both subscribes to both
...
- **Client-relay handshake** — version negotiation is part of the `ClientHello`/`RelayHello` exchange (§8.6)
- **Relay-to-relay Peer API** — version is in the URL path; manifest `capabilities` array declares which versions a relay supports

---

## 1. Overview

Agora is a decentralized, end-to-end encrypted realtime group chat protocol. It is designed to support a user experience comparable to Discord — servers, channels, presence, typing indicators, voice/video signaling — without any central authority owning identity, routing, or message storage.
...
Agora does **not** define a central server. It defines a protocol that servers (called **Relays**) can implement to form a federated mesh. Users are not locked to any Relay. Relays gossip with each other. Clients can connect to multiple Relays simultaneously and reconcile state.
...
`recoverySig` is signed by the recovery key. `revokeAll: true` instructs relays and peers to treat all previously registered device keys as revoked. The `RecoveryAssertion` is published to `v1/agora/discovery` and stored in the user's profile IPLD chain. Relays that see a valid `RecoveryAssertion` MUST invalidate all cached sessions for that DID immediately.
...
Social recovery designates a set of **guardians** — other Agora users trusted to collectively authorize identity recovery. A threshold scheme (e.g. 3-of-5) means no single guardian can unilaterally recover the account, and loss of any one guardian does not prevent recovery.
...
**IPv6** — All relay endpoints SHOULD support IPv6. Relay URIs MUST use bracket notation for IPv6 literals (`wss://[2001:db8::1]/v1/agora/ws`). ICE gathers both IPv4 and IPv6 host candidates; dual-stack clients race both address families.
...
| `peer-api-v1` | Implements Peer API v1 (§3.5.2) |
| `push-proxy-v1` | Designates a push proxy (§8.7) |
...
Relay-to-relay requests use **RFC 9421 HTTP Message Signatures** with Ed25519. The requesting relay signs each HTTP request using its active signing key from its manifest. The receiving relay verifies the signature against the requesting relay's manifest fetched from `/.well-known/agora-relay`.
...
signature-input: sig1=("@method" "@target-uri" "@authority"   "x-agora-relay-did" "x-agora-timestamp" "x-agora-nonce");   keyid="did:key:z6MkRelay...#key-1"; alg="ed25519"
signature: sig1=:base64url...:
```

**Replay prevention:** `x-agora-timestamp` MUST be within 300 seconds of the receiving relay's clock. `x-agora-nonce` MUST NOT have been seen in the last 600 seconds (receiving relay maintains a nonce cache with 600s TTL). Requests violating either condition MUST be rejected with HTTP 401.
...
**Method 1 — Known peer URL.** The operator provides one or more peer relay URLs in startup configuration. The new relay fetches `/.well-known/agora-relay`, verifies the manifest signature, sends `POST /peer/v1/announce` with its own manifest, and on acceptance fetches `GET /peer/v1/peers` to discover further relays.

**Method 2 — Directory document.** The relay fetches any Agora relay directory document (§4.4.1) and contacts listed relays via Method 1.
...
**Method 4 — IPNS fallback.** The relay resolves `/ipns/agora.protocol/relays/v1` for a community-maintained signed bootstrap list. Last resort.
...
Agora uses **libp2p gossipsub v1.1** for:
...
| `v1/agora/guild/<guildCID>` | Guild-scoped events (joins, state updates) |
| `v1/agora/channel/<channelCID>` | Per-channel messages and presence |
| `v1/agora/mls/<groupID>` | MLS handshake messages (Welcome, Commit, Proposal) |

The `v1/` prefix is the gossip protocol version for that topic family. If a future version introduces a breaking change to message framing on a topic, the new topic string uses a `v2/` prefix (e.g., `v2/agora/channel/<channelCID>`). Relays MUST subscribe to all versions of topics they serve. Clients negotiate which version to publish on via the `negotiated.gossipVersion` field in `RelayChallenge` (§8.6). During a version transition period, relays bridge messages between `v1/` and `v2/` topic variants for the same channel.
...
4. **DNS-SD** — `_agora._tcp` mDNS for LAN/overlay discovery
...
Agora uses **best-effort causal ordering**. Gossipsub does not guarantee total order and does not need to. The requirement is that messages arrive in approximately the right order on average, with clients able to reconstruct causal order locally.
...
Presence is gossiped on `v1/agora/channel/<channelCID>` as a `PresenceEvent`:
...
`version` values: `0x01` = frame format v1 (this specification). A relay that receives a frame with an unknown `version` MUST discard it and MAY close the connection. Future frame format versions increment this byte.
...
Push proxies are entirely optional. Relays MAY designate one; clients MAY register with one. Clients that maintain persistent connections (desktop, server-side bots, always-on mobile) do not need them.

#### 8.7.1 Privacy Model

The fundamental constraint: APNs (Apple) and FCM (Google) are centralized services that require a server-side component knowing which device token to wake up. This server-side component is the push proxy. The goal is to limit what the proxy learns to the minimum necessary for its function.

**What the push proxy learns:**
- A client's opaque `pushHandle` (random token, not linked to DID or channel)
- The client's APNs/FCM device token (required to send the push)
...
- The client's APNs/FCM device token (never transmitted to the relay)
- The push proxy's internal mapping

The `pushHandle` is the separation layer. It is generated fresh by the client on each app install. The relay knows DID → pushHandle. The proxy knows pushHandle → device token. Neither has both. A relay cannot identify which physical device a DID maps to; a proxy cannot identify which DID a device token belongs to.
...
A relay MAY run its own push proxy or point to a third-party proxy. Multiple relays MAY share a push proxy. The `proxyDID` identifies the proxy as a DID-bearing principal — the relay authenticates to it using the same RFC 9421 signed request mechanism as relay-to-relay auth (§3.5.3).

#### 8.7.3 Client Registration

On first launch on a mobile device, a client that wishes to receive push notifications:

1. Generates a random 256-bit `pushHandle` (stored locally, never shared with the relay directly)
...
The proxy stores `pushHandle → { deviceToken, platform, appBundleID }`. It does not store the client's DID or any channel information. The registration is unauthenticated from the proxy's perspective — any bearer of the `pushHandle` can update or delete it. The `pushHandle` itself is the credential.

4. Registers `{ pushHandle, proxyEndpoint }` with their relay, authenticated via the session token from §8.6:

```
POST /v1/agora/kp/../push/register
     (or a dedicated endpoint on the relay)
{
...
The relay stores `sessionDID → { pushHandle, proxyEndpoint }`. It uses this to send wake-up requests when the client is offline.

**Registration renewal:** Push registrations have a TTL (default 30 days). Clients MUST renew before expiry. APNs/FCM device tokens can also change (OS reinstall, token rotation); clients MUST re-register with the proxy when they receive a new device token.
...
The request is signed by the relay using its manifest key (§3.5.3). The body contains only the `pushHandle` and an urgency hint. No channel, no sender, no content.

3. The proxy maps `pushHandle → deviceToken` and sends a zero-content push notification to APNs/FCM:
...
Both payloads are content-free silent pushes. APNs `content-available: 1` triggers a background app refresh; FCM `data`-only messages do the same on Android. The OS wakes the app; the app connects to its relay; the relay delivers the pending messages over the authenticated session.

4. The OS launches the app in the background. The client reconnects to its relay, completes the auth handshake (§8.6), and receives queued messages normally.

`urgency: "high"` is used for direct messages or @mentions and results in a high-priority push that may produce an audible notification. `urgency: "normal"` is used for general channel traffic and uses silent background push only. The relay determines urgency based on the message's `mentions` field (inner payload, which the relay cannot read) — since the relay cannot inspect sealed content, urgency is determined by a hint the sending client attaches to the outer `RoutingEnvelope`:

```json
"pushUrgency": "high" | "normal"
```

This field is unauthenticated and advisory — relays use it as-is. A malicious sender setting all messages to `high` urgency wastes push quota; the per-channel rate limits in §13.5 bound the damage.
...
Proxies SHOULD rate-limit wake-up requests per `pushHandle` to prevent a relay from spamming a client device (e.g., 1 push per 5 seconds per handle, with burst allowance of 3). Excess wake-up requests within the window are silently dropped — the client will connect on the next successful push or on next foreground.

#### 8.7.6 Handle Rotation

A client MAY rotate its `pushHandle` at any time (e.g., periodically for privacy, or after suspected handle compromise). Rotation procedure:

1. Generate new `pushHandle`
2. Register new `{ pushHandle, deviceToken }` with proxy
3. Register new `{ pushHandle, proxyEndpoint }` with relay (relay replaces old handle)
4. Delete old `pushHandle` from proxy: `DELETE /v1/agora/push/register/{pushHandle}`

The relay atomically replaces the old handle with the new one. There is a brief window between step 2 and step 3 where wake-ups to the old handle still work; this is acceptable. After step 4, the old handle is invalid and the proxy discards any wake-up requests for it.
...
Agora supports real-time group voice and video conferencing as a first-class channel type. The design uses WebRTC for media transport, MLS-encrypted signaling for all control messages, and an optional SFU for scalable multi-party sessions. Media never transits Relays.
...
In Agora, the SDP carrying the fingerprint is delivered as an MLS `VoiceSignal` inner payload, signed by the sender's DID authentication key. Recipients verify the MLS signature before processing the SDP. This means:
...
TURN servers in Agora have DIDs, published in the Guild state document alongside SFUs:
...
- Announce itself on `v1/agora/discovery` with a signed `RelayAd` including all available endpoint URIs (clearnet, `.onion`, overlay-internal)
...
All payment destinations in Agora are expressed as a `PaymentPointer` — an abstract type carrying a scheme identifier and a scheme-specific address. The protocol is not coupled to any single payment network; new schemes are registered via the Agora extension namespace.
...
The agreement is gossiped on `v1/agora/guild/<guildCID>` and stored in the guild's IPLD state. Multiple competing agreements (multiple relays serving the same guild) are valid and encouraged for redundancy.
...
It maps all Agora message types and properties to globally unique IRIs and declares their relationship to relevant external vocabularies (schema.org, W3C DID, ActivityStreams where applicable).

All Agora messages MUST include `"@context": "https://agora.protocol/ns/v1"` or an equivalent inline context. Processors that do not perform full JSON-LD expansion MAY treat the context as a version tag, but MUST NOT reject messages that include additional `@context` entries for extension vocabularies.

The context URL is versioned: `/ns/v1` is the v1 schema. A breaking change to the type system produces `/ns/v2`. The context URL is frozen once published — `/ns/v1` will always describe the v1 schema without modification. Documents using `/ns/v1` types remain valid indefinitely against that context. A document using `/ns/v2` types in a `/ns/v1` context MUST include the v2 context as an additional `@context` entry; processors that understand only v1 treat unknown types as per the extension point rules in §15.
...
1. **Expand** the document using the Agora JSON-LD context (`https://agora.protocol/ns/v1`), resolving all terms to absolute IRIs. Use a strict JSON-LD 1.1 processor. Unknown terms MUST be dropped during expansion, not passed through.
2. **Serialize** the expanded document to RDF N-Quads (one quad per line, no blank line at end).
3. **Canonicalize** the N-Quads dataset using **URDNA2015** (W3C RDF Dataset Normalization Algorithm, also referred to as RDNA). This deterministically renames blank nodes and sorts quads, producing a stable byte string regardless of input key ordering or whitespace.
...
This pipeline is identical to **W3C Data Integrity Proofs** with the `eddsa-rdna-2022` cryptosuite, which is the signing layer used by the W3C Verifiable Credentials ecosystem and compatible with `did:key` and `did:web` document signing. Implementations SHOULD use an existing conformant library rather than implementing the pipeline from scratch.

**Reference implementations:**
- TypeScript/JS: `jsonld` + `rdf-canonize` (Digital Bazaar) — these are the W3C reference implementations
- Go: `go-jsonld-signatures` or equivalent binding to the above via FFI/subprocess for correctness during initial development
- Rust: `json-ld` crate + `rdf-types` + `ssi` crate (Spruce Systems)
...
Because URDNA2015 requires a JSON-LD expansion step, implementations need access to the context document at `https://agora.protocol/ns/v1`. Fetching this document at signing/verification time is unacceptable for latency and offline operation.
...
The pinned hash for `/ns/v1` will be published alongside the v1 context document at spec finalization. During development, implementations SHOULD use the context document from the Agora repository at a pinned commit rather than fetching from the canonical URL.

Implementations MUST NOT fetch the context document from the network during signature verification of an untrusted message — this is a denial-of-service vector. The context document MUST be loaded from the local cache only.
...
The credential is signed by the Agora authentication key. MIMI clients verify the MIMI URI portion; Agora clients verify the DID portion. Neither side needs to understand the other's identifier format beyond treating it as an opaque string for display purposes.
...
**MIMI content format mode** — the channel uses the MIMI content format as the canonical inner payload. Agora clients serialize outgoing messages to MIMI content format and deserialize incoming MIMI content to their local display model. Agora-specific features not representable in MIMI content format (nested channel references, Agora-specific reaction types) are carried as MIMI content extensions using the MIMI extension mechanism.
...
Matrix interop is a **gateway**, not native interop. A Matrix gateway holds an Agora DID, is admitted to the relevant channel MLS groups as a member, decrypts Agora messages, re-encrypts them using Matrix's Megolm E2EE for delivery to Matrix users, and performs the reverse for Matrix→Agora direction. E2EE is broken at the gateway — the gateway holds plaintext. This MUST be made visible to all participants (see §16.3).
...
1. Gateway receives MLS-encrypted message from Agora channel gossip topic
...
4. Gateway sends translated event to the bridged Matrix room via the Matrix Client-Server API, attributed to a virtual Matrix user representing the Agora sender (`@agora_z6Mk...:example.com`)
...
2. Gateway translates Matrix event to Agora inner payload format
...
5. Agora clients see the message as coming from the gateway DID with a `bridgedFrom` field in the inner payload identifying the original Matrix sender's MXID
...
Agora DIDs map to Matrix virtual users with a deterministic MXID derived from the DID: `@agora_<did-fragment>:<gateway-homeserver>`. Matrix MXIDs map to Agora gateway messages with `bridgedFrom.senderID` carrying the MXID. Display names are fetched from Agora user profiles and Matrix profiles respectively and cached by the gateway.

#### Namespace mapping

Agora's nested channel namespace does not map cleanly to Matrix's flat room model. The gateway maps each Agora channel path to a separate Matrix room. A guild with nested channels produces a Matrix Space containing one room per channel:
...
When the Matrix room has Megolm E2EE enabled, the gateway holds both MLS session state (for Agora) and Megolm session state (for Matrix). The gateway is the trust boundary — it decrypts on one side and re-encrypts on the other. This is unavoidable; it is the nature of a gateway between two incompatible E2EE systems.
...
When the Matrix room does not have Megolm E2EE enabled, the gateway sends plaintext to the Matrix server. This is a further trust degradation — the Matrix homeserver operator can read bridged messages. This MUST be disclosed to Agora channel members (see §16.3).
...
Existing Matrix bridge frameworks (mautrix-go, mautrix-python) provide the application service scaffolding. The Agora-specific work is the MLS client integration, the Agora↔Matrix content format translation, and the IPFS↔Matrix media re-hosting.

---

### 16.3 Gateway Transparency

Any gateway admitted to an Agora channel — Matrix, or any future gateway — MUST be disclosed to all channel members. This is enforced at the protocol level, not by policy.
...
1. **KeyPackage distribution** — RFC 9420 requires a reliable KeyPackage store. Agora uses IPFS for this but does not specify pinning guarantees. A Relay-hosted KeyPackage endpoint with explicit availability SLA may be necessary for reliability in practice.
