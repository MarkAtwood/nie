...
All versioned protocol surfaces in this specification are at **v1** for the initial release. The version appears as a path prefix on HTTP endpoints (`/v1/rlchat/...`) and as a topic prefix on gossip topics (`v1/rlchat/...`), JSON-LD context URLs (`/ns/v1`), and document `schemaVersion` fields. A breaking change to any surface increments only that surface's version; other surfaces are unaffected. Clients and relays MUST reject messages or connections whose version they do not support and MUST NOT silently coerce unknown versions.
...
`recoverySig` is signed by the recovery key. `revokeAll: true` instructs relays and peers to treat all previously registered device keys as revoked. The `RecoveryAssertion` is published to `v1/rlchat/discovery` and stored in the user's profile IPLD chain. Relays that see a valid `RecoveryAssertion` MUST invalidate all cached sessions for that DID immediately.
...
**IPv6** — All relay endpoints SHOULD support IPv6. Relay URIs MUST use bracket notation for IPv6 literals (`wss://[2001:db8::1]/v1/rlchat/ws`). ICE gathers both IPv4 and IPv6 host candidates; dual-stack clients race both address families.
...
```json
{
  ...
  "region": "us-west",
  "endpoints": {
    "webTransport": "https://relay.example.com/v1/rlchat/wt",
    "webSocket": "wss://relay.example.com/v1/rlchat/ws",
    "onion": "ws://examplerelay3xyzabc.onion/v1/rlchat/ws",
    "keyPackageStore": "https://relay.example.com/v1/rlchat/kp",
    "peerAPI": "https://relay.example.com/v1/rlchat/peer"
  },
  "keys": [
    {
...
|---|---|
| `v1/rlchat/discovery` | Global guild/user advertisements |
| `v1/rlchat/guild/<guildCID>` | Guild-scoped events (joins, state updates) |
| `v1/rlchat/channel/<channelCID>` | Per-channel messages and presence |
| `v1/rlchat/mls/<groupID>` | MLS handshake messages (Welcome, Commit, Proposal) |

The `v1/` prefix is the gossip protocol version for that topic family. If a future version introduces a breaking change to message framing on a topic, the new topic string uses a `v2/` prefix (e.g., `v2/rlchat/channel/<channelCID>`). Relays MUST subscribe to all versions of topics they serve. Clients negotiate which version to publish on via the `negotiated.gossipVersion` field in `RelayChallenge` (§8.6). During a version transition period, relays bridge messages between `v1/` and `v2/` topic variants for the same channel.
...
- `Welcome` — sent to new members via `v1/rlchat/mls/<groupID>` or direct encrypted delivery
...
```
GET /v1/rlchat/kp/{did}?forward=true
```
...
2. The sender publishes a `KeyPackageRequest` to `v1/rlchat/discovery`:
...
Presence is gossiped on `v1/rlchat/channel/<channelCID>` as a `PresenceEvent`:
...
Endpoint: `https://<relay-host>/v1/rlchat/wt`
...
Endpoint: `wss://<relay-host>/v1/rlchat/ws`
...
```json
"pushProxy": {
  "endpoint": "https://push.example.com/v1/rlchat/push",
  "proxyDID": "did:key:z6MkProxy...",
  "platforms": ["apns", "fcm"],
  "registrationEndpoint": "https://push.example.com/v1/rlchat/push/register"
}
```
...
```
POST /v1/rlchat/kp/../push/register
     (or a dedicated endpoint on the relay)
{
  "pushHandle": "base64url...",
  "proxyEndpoint": "https://push.example.com/v1/rlchat/push"
}
```
...
```
POST https://push.example.com/v1/rlchat/push/wake
x-rlchat-relay-did: did:key:z6MkRelay...
x-rlchat-timestamp: 1741780800
x-rlchat-nonce: base64url...
signature: sig1=:base64url...:
...
4. Delete old `pushHandle` from proxy: `DELETE /v1/rlchat/push/register/{pushHandle}`
...
All moderation operations are cryptographically signed and gossip-propagated on `v1/rlchat/guild/<guildCID>`. Relays that serve the affected guild MUST enforce them on receipt of a valid signed record.
...
Call state is ephemeral, gossip-propagated, and not stored in IPLD history. It is maintained as a set of `ParticipantState` records, one per active participant, gossiped on `v1/rlchat/channel/<channelToken>` alongside presence events.
...
"sfus": [
  {
    "@type": "SFUDescriptor",
    "did": "did:key:z6MkSFU...",
    "endpoint": "wss://sfu.example.com/v1/rlchat/sfu",
    "regions": ["us-west", "eu-central"],
    "maxParticipants": 500
  }
]
...
The TURN server's published HMAC key material is signed by its DID and gossiped on `v1/rlchat/guild/<guildCID>`. Clients verify the signature against the TURN server's DID document before using its credentials.
...
- Announce itself on `v1/rlchat/discovery` with a signed `RelayAd` including all available endpoint URIs (clearnet, `.onion`, overlay-internal)
...
- Offer an HTTP API for history fetch (`GET /v1/rlchat/history/<channelToken>?before=<cid>&limit=50`)
...
The agreement is gossiped on `v1/rlchat/guild/<guildCID>` and stored in the guild's IPLD state. Multiple competing agreements (multiple relays serving the same guild) are valid and encouraged for redundancy.
...
| WebTransport endpoint | v1 | URL path prefix `/v1/rlchat/wt` | HTTP 404 on unknown version |
| WebSocket endpoint | v1 | URL path prefix `/v1/rlchat/ws` | HTTP 404 on unknown version |
...
| KeyPackage Store API | v1 | URL path prefix `/v1/rlchat/kp/` | HTTP 404 on unknown version |
| Peer API | v1 | URL path prefix `/v1/rlchat/peer/` + `peer-api-v1` capability | HTTP 404; capability string |
| Push Proxy API | v1 | URL path prefix `/v1/rlchat/push/` + `push-proxy-v1` capability | HTTP 404; capability string |
...
```json
{
  ...
  "service": [{
    ...
    "serviceEndpoint": "https://archive.acme-corp.internal/v1/rlchat/compliance",
...
The agreement is stored as an IPLD node, CID-referenced from both guild state documents, and gossiped on both `v1/rlchat/guild/<homeGuildCID>` and `v1/rlchat/guild/<guestGuildCID>`.
...
The home channel's gossip topic (`v1/rlchat/channel/<channelToken>`) is where all messages flow. Guest guild members subscribe to this topic via their own Relays. For this to work, the guest guild's Relays must be peered with (or able to reach) the home channel's Relays.
