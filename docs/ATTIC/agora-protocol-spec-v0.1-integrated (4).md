...
2. On receiving a valid `RecordingGrant`, each participant's client derives a shared recording key using the MLS exporter interface (RFC 9420 §8.5) — `MLS-Exporter("agora-recording-key", context, 32)` — and delivers it to the SFU via an encrypted direct message addressed to the SFU's DID. The derivation occurs inside the MLS stack; the epoch secret itself is never exposed to application code.
