...
When compliance logging is enabled for a guild (your organization's Agora workspace), a designated **compliance logger** — a cryptographically-credentialed system principal — is automatically added as a silent member of every relevant message group. It receives and archives:
...
- **All text messages** in covered channels, in their original form, including edits (the edit history is preserved as separate records) and deletions (tombstone records with the deletion timestamp and the deleting user's identity)
- **All file attachments and media**, stored by content hash (CID) in your archive — the same hash as the original, so authenticity is mathematically verifiable
...
- **Messages in channels not covered by the scope configuration** — if your `scope` is configured as a channel path glob (e.g., `engineering/*`) rather than `all-channels`, messages in unscoped channels are not captured. Review your scope configuration carefully.
- **Messages from non-regulated employees in DM groups**, prior to the regulated employee joining — once a regulated employee joins a DM group, the logger is added at that point; messages from before their join are not retroactively accessible.
...
- A link to the prior record in the chain (`prevRecordCID`) — this chain structure means any alteration or deletion of a record is immediately detectable by verifying the chain hashes
...
**Authenticity verification.** Because each `ComplianceRecord` is cryptographically signed by the logger, and the records form a hash-linked chain, you can produce records in an eDiscovery proceeding and demonstrate mathematically that they have not been altered since capture. This is stronger than most conventional archiving solutions, which rely on access controls rather than cryptographic proof.

---

## Part 2: Direct Message Compliance

This is where most compliance programs have historically had gaps, and where Agora's architecture is most directly relevant to current regulatory focus.

### The obligation

A regulated employee (a member of a guild with compliance logging enabled) **carries their compliance obligation into every direct message group they participate in**, regardless of whether the DM group has any connection to the organization's guild infrastructure.

This is not optional and it is not configurable. The Agora specification (§17.8.1) states:
...
2. Adds the logger to the DM group's MLS group via a cryptographic commit — the logger becomes a full member with decryption rights
3. Updates the DM group's descriptor to record the logger's presence
4. Sends a **mandatory disclosure notice** to all other DM group participants before any message is sent

The disclosure notice is a visible system message (not a suppressable notification) that tells all participants:
...
Non-regulated participants — counterparties, clients, external contacts — are notified before they send any message. They may leave the DM group at that point if they choose. Their messages sent before their departure remain in the archive for the regulated employee's retention period.

### Logger removal protection

Once the compliance logger is admitted to a DM group, it cannot be removed for the duration of the retention period. Specifically:

- The regulated employee's client will refuse to process any request to remove the logger
- If another DM group member (a non-regulated counterparty) somehow removes the logger, the regulated employee's client immediately detects this, re-adds the logger, and **automatically notifies the compliance officer and guild admin** of the removal attempt

The notification of removal attempts is recorded in the organization's audit chain as a `DMComplianceAuditEntry` with the timestamp and identity of who attempted the removal.

### What this means for your gap analysis

**The gap Agora closes:** FINRA and SEC examiners have, in recent examination cycles, focused heavily on business communications conducted via personal devices and messaging applications (WhatsApp, Signal, iMessage, personal SMS) that are not captured by firm archiving systems. The standard approach — policy prohibition — has proven ineffective. Agora's DM compliance architecture makes capture automatic and mandatory at the client level for regulated employees, rather than relying on employee self-reporting or retrospective device examination.

**The gap Agora does not close:** If a regulated employee uses a non-Agora application (WhatsApp, personal email, personal Signal) for business communications, Agora cannot capture it. No communication protocol can enforce capture of communications that don't use it. Agora eliminates the gap for employees who use Agora for DMs; it does not eliminate the gap created by employees who use other applications.

**The practical implication:** For Agora to close the DM gap, the employee and their counterparties must both be Agora users. This is most tractable for intra-firm communications and for firms that can contractually require counterparties to use Agora-compliant communication channels.
...
- A set of rotating, anonymous channel tokens — not linked to any channel name, guild, or identity without the epoch secret (which the relay does not hold)
- A set of encrypted message blobs addressed by content hash
- Connection metadata: IP addresses and timestamps of connected clients — but only during the connection; no persistent connection log is required by the protocol
...
- Message history beyond what it has chosen to cache (relay caching is configurable)

**Practical consequence for legal hold:** If you receive a third-party subpoena to a relay operator serving your organization, the relay operator's honest response is that they hold encrypted blobs they cannot read and do not know who sent them. This is not a legal opinion — it is a description of what the relay holds. Whether this satisfies the subpoena depends on jurisdiction and the specific legal process; consult counsel.
...
1. **Identify the relevant DIDs.** Each employee corresponds to one or more DIDs. The mapping from employee name to DID is maintained in your identity management system (or derivable from your guild's member list and DID documents).

2. **Suspend archive pruning for affected DIDs and channels.** Your compliance archive is append-only by design — records are not automatically deleted. But if your archive configuration includes a purge policy (e.g., records older than `retentionDays` are deleted after the retention period expires), you must suspend that purge for held content.
...
4. **Verify chain integrity.** Before producing records, verify that the `prevRecordCID` chain is intact for the exported range. A broken chain indicates a record was altered or deleted — this would need to be investigated and disclosed.

5. **Preserve the `DMComplianceAuditEntry` records** for any DM groups involving the relevant employees. These records document logger presence and any removal attempts.
...
Employees in a compliance-logging-enabled guild receive a mandatory disclosure at first join:

> "This guild is subject to regulatory message retention. Messages are archived for [N] days regardless of expiry settings."

This notice is displayed by the client and cannot be dismissed without acknowledgment. It is also accessible at any time from the guild's information panel.

**DM-specific disclosure.** When a regulated employee's client adds the compliance logger to a DM group, a `ComplianceDisclosureNotice` is sent to all DM participants as a visible system message before any application message is sent. This notice identifies the logger by label (e.g., "Acme Corp Compliance Archive"), states the retention period, and lists the regulatory frameworks under which the logger operates.

### Counterparty disclosure

External counterparties who receive DMs from regulated employees will see the compliance disclosure notice before they send any message. They are not asked to consent — the disclosure is informational. They may leave the DM group if they choose. This mirrors the existing practice of recording disclosures on regulated telephone calls ("this call may be recorded for compliance purposes") but applies it at the messaging layer.
...
**Capture scope**
- [ ] `complianceLogging.enabled` is `true` in the guild state document
...
- [ ] The logger is operated in a mode that prevents deletion of records within the retention period — the append-only chain structure helps, but your operational controls must prevent administrative deletion of the underlying IPFS data

**Audit and monitoring**
- [ ] `DMComplianceAuditEntry` records are being produced and accessible to your compliance team
...
- [ ] A process exists to map DID → employee name for eDiscovery production

**Disclosure**
- [ ] `retentionNotice` text in the guild state document is accurate and legally reviewed
...
**VTC compliance**
- [ ] `vtcCompliance.enabled` is `true` in the guild state document if voice channels are used for regulated communications
...
**Gaps to acknowledge in writing**
...
| GDPR Article 17 (right to erasure) | Ability to delete personal data on request | Records in compliance archive cannot be deleted during retention period — this creates direct tension with erasure rights for regulated communications. Standard exemption for legal retention obligations applies; document the basis. | Legal analysis required per jurisdiction |
