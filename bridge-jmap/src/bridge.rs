...
use nie_core::protocol::{
    rpc_methods, BroadcastParams, DeliverParams, JsonRpcRequest, PublishHpkeKeyParams,
};
                ...
                match event {
                    ClientEvent::Message(notif) => {
                        if notif.method == rpc_methods::DELIVER {
                            ...
                            .await;
                        } else if notif.method == rpc_methods::BROADCAST {
                            // MLS group messages arrive as BROADCAST. bridge-jmap has no
                            // MlsClient, so decryption is not supported. Log at warn so
                            // operators can see messages are arriving but not forwarded.
                            tracing::warn!(
                                "bridge-jmap: received BROADCAST (MLS group message) but \
                                 MLS decryption is not supported; message not forwarded to JMAP"
                            );
                        }
                    }
                    ...
                    ClientEvent::Reconnected => {
                        tracing::info!("relay reconnected; republishing HPKE key");
                        // The relay clears HPKE key slots on restart. Re-publish so
                        // peers can continue to send sealed messages to this bridge.
                        match JsonRpcRequest::new(
                            ...
                            Ok(req) => {
                                if conn.tx.send(req).await.is_err() {
                                    tracing::warn!(
                                        "bridge-jmap: relay_tx closed while republishing HPKE key"
                                    );
                                } else {
                                    tracing::info!("bridge-jmap: republished HPKE key after reconnect");
                                }
                            }
                            Err(e) => {
                                tracing::warn!(
                                    "bridge-jmap: failed to build PUBLISH_HPKE_KEY request: {e}"
                                );
                            }
                        }
                    }
